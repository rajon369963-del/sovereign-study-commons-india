#!/usr/bin/env python3
"""Physical kill-mutant court for Issue #99 / PR #101 lineage.

The discriminator court is mechanism-sensitive only if bypassing the named
commit/tree/script validator makes the corresponding targeted test go RED.
This court loads temporary copies of the production verifier, bypasses exactly
one target guard at a time, and requires the original discriminator assertion
to fail against that mutant.
"""
...
def test_commit_reachability_guard_bypass_kills_commit_discriminator(tmp_path):
    ...
    _assert_existing_test_goes_red(mutant, "test_fake_commit_reaches_commit_reachability_validator")
...
def test_exact_tree_guard_bypass_kills_tree_discriminator(tmp_path):
    ...
    _assert_existing_test_goes_red(mutant, "test_fake_tree_reaches_exact_tree_validator")
...
def test_script_digest_guard_bypass_kills_script_discriminator(tmp_path):
    ...
    _assert_existing_test_goes_red(mutant, "test_tampered_script_digest_reaches_script_digest_validator")
