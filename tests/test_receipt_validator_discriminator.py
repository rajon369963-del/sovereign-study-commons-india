...
def _run_with_post_auth_view(mutator):
    ...
    verify() parses the receipt once into `data`, then reparses the untouched raw
    bytes into `clean_copy` for canonical payload hashing/signature verification.
    We mutate only that first parsed object. Thus the real pinned-key, payload-hash,
    and Ed25519 checks still operate on pristine signed bytes, while downstream
    commit/tree/script validators receive the hostile telemetry view.
    """
    ...
    def bounded_check_call(cmd, *args, **kwargs):
        # A fake commit should fail at the reachability validator without a network wait.
