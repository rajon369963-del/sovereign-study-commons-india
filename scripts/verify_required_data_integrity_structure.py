#!/usr/bin/env python3
"""Bounded cross-guard for the required Data integrity workflow.

This deliberately avoids a YAML dependency. It recognizes only the small workflow
shape we own: one named job and named steps with active ``run:`` keys. The guard
is hosted by the independently required SHA256 idempotency canary so Data
integrity cannot attest the presence of its own manifest-enforcement steps.
"""
...
TARGET_JOB_NAME = "Data integrity"
REQUIRED_STEPS = {
...
def _active_named_steps(job_lines: list[str]) -> dict[str, str]:
    ...
    for position, start in enumerate(starts):
        ...
        if not raw or raw in {"|", ">", "|-", ">-"}:
            # Multiline/dead text is intentionally not accepted for these exact
            # single-command policy steps.
            continue
...
def assert_required_structure(text: str) -> None:
    ...
    if f"    name: {TARGET_JOB_NAME}" not in job:
        raise StructureError(
            f"required job {TARGET_JOB!r} must emit check name {TARGET_JOB_NAME!r}"
        )
