...
VERIFIED_PROSE_RES = [
    ...
    re.compile(
        rf"\b(?:all\s+)?({COUNT_TEXT})\s+({FAMILY_TEXT})\s+(?:(?:were|are)\s+(?:all\s+)?{STATUS_ADVERB_TEXT}|have\s+(?:(?:all\s+)?been\s+{STATUS_ADVERB_TEXT}|(?:independently|fully)\s+been\s+))?{STATUS_TEXT}\b",
        re.IGNORECASE,
    ),
    re.compile(
