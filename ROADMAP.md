...
- Pull request #5 is open and unmerged. Its current head is `7a694712e81742730b19e7949de82002eb92341e`; the latest run (`34236376956`) is **RED** at `Verify SQLite and Parquet integrity`. The branch-only workflow contains fail-closed FTS membership and duplicate-ID assertions. That red run is evidence that the current contract is not satisfied; it is not merge readiness and must not be weakened merely to recover green.
- A sibling diagnostic points to one missing FTS member (`GATE-EE-COM-004` / rowid 11) as a likely source/index synchronization seam. AIR10-03 does not treat that causal explanation as independently reproduced repo truth yet; repair promotion remains blocked until an independent verifier reproduces the mismatch from the current head.
